...
From Stdlib Require Import Setoid CMorphisms BinPos BinNat.
...
Section DEFINITIONS.
 ...
 (** Semi Ring *)
 Record semi_ring_theory : Type := mk_srt {
...
(*Almost ring are no ring : Ropp_def is missing **)
 Record almost_ring_theory : Type := mk_art {
...
(*for semi rings*)
 Record semi_morph : Type := mkRmorph {
    Smorph0 : [cO] == 0;
    Smorph1 : [cI] == 1;
...
(* for rings*)
 Record ring_morph : Type := mkmorph {
    morph0    : [cO] == 0;
    morph1    : [cI] == 1;
 ...
 Section SIGN.
  ...
  Record sign_theory : Set := mksign_th {
 ...
 Section DIV.
  ...
  Record div_theory : Type := mkdiv_th {
 ...
 Section POWER.
  ...
  Record power_theory : Type := mkpow_th {
...
Section ALMOST_RING.
 ...
 Instance radd_ext1 : Proper (req ==> req ==> req) radd.
 Proof. exact (SRadd_ext SReqe). Qed.

 Instance rmul_ext1 : Proper (req ==> req ==> req) rmul.
 ...
 Instance radd_ext2 : Proper (req ==> req ==> req) radd.
 Proof. exact (Radd_ext Reqe). Qed.

 Instance rmul_ext2 : Proper (req ==> req ==> req) rmul.
 Proof. exact (Rmul_ext Reqe). Qed.

 Instance ropp_ext2 : Proper (req ==> req) ropp.
...
Admitted.
 ...
 Variable C : Type.
 Variable (cO cI : C) (cadd cmul csub: C->C->C) (copp : C -> C).
 Variable (ceq : C -> C -> Set) (ceqb : C -> C -> bool).
 Variable phi : C -> R.
 ...
 Instance C_setoid : @Equivalence C ceq.
 Proof.
   constructor.
   exact (@Equivalence_Reflexive _ _ Csth).
   exact (@Equivalence_Symmetric _ _ Csth).
   exact (@Equivalence_Transitive _ _ Csth).
 Qed.

 Instance cadd_ext : Proper (ceq ==> ceq ==> ceq) cadd.
 Proof. exact (Radd_ext Ceqe). Qed.

 Instance cmul_ext : Proper (ceq ==> ceq ==> ceq) cmul.
 Proof. exact (Rmul_ext Ceqe). Qed.

 Instance copp_ext : Proper (ceq ==> ceq) copp.
 ...
 Instance phi_ext1 : Proper (ceq ==> req) phi.
